...
class MeteorObservation:
    ...
    @classmethod
    def from_meteor_candidate(cls: Type[T], candidate: Any) -> T:
        """
        从 MeteorCandidate 对象（通常是上游处理流程的输出）创建 Observation 实例。

        Args:
            candidate: 包含流星候选数据的对象。应包含 stationCode, camID, data, pipeline 等属性。

        Returns:
            MeteorObservation: 初始化后的观测对象。
        """
        met_obs = cls()
        # 版本
        met_obs.version = "0.1"
        # 基础
        met_obs.station_name = candidate.stationCode
        met_obs.camera_name = candidate.camID
        met_obs.meteor_name = candidate.name
        met_obs.mode = candidate.mode
        met_obs.station_location = candidate.stationLoc
        met_obs.color = candidate.color
        # 阶段
        met_obs.contents = ["image", "time"]
        if candidate.pipeline == "basic":
            met_obs.contents += []
        if candidate.pipeline == "full":
            met_obs.contents += ["meteor", "star", "calibration"]
        met_obs.processed_time = candidate.processed
        # 图像
        met_obs.image_width = candidate.w
        met_obs.image_height = candidate.h
        met_obs.frame_count = candidate.framecount
        met_obs.data = candidate.data
        met_obs._mean_frame = candidate.meanframe_data
        met_obs.signal_frame = candidate.sigframe
        met_obs._std_frame = candidate.stdframe_data
        met_obs._max_frame = candidate.maxframe_data
        met_obs.mask_frame = candidate.maskframe
        # 时间
        met_obs.time_method = "m3"
        met_obs.time_source = candidate.timesource
        met_obs.mean_time = candidate.meantime
        met_obs.frame_time = np.array(candidate.frametime)
        met_obs.frame_exposure = np.array(
            [
                candidate.camstop[i] - candidate.camstart[i]
                for i in range(candidate.framecount)
            ]
        )
        # 恒星
        met_obs.star_method = "m3"
        met_obs.star_pixel_coord = candidate.star_pic
        met_obs.star_eci_coord = candidate.star_eci
        met_obs.star_name = [s for s in candidate.star_name]
        met_obs.star_magnitude = None
        met_obs.star_flux = None
        # 流星
        met_obs.object_method = "m3"
        met = ObservationObject()
        met.meteor_index = candidate.metindex
        met.meteor_pixel_coord = candidate.metpic
        met.meteor_eci_coord = candidate.meteci
        met.object_type = candidate.objtype
        met_obs.objects = [met]
        # 校准
        met_obs.calibration_method = "m3"
        met_obs.calibration_result = candidate.calib_result
        model: Dict[str, Union[float, Any]] = {}
        for i in range(len(candidate.npf)):
            model[f"npf{i}"] = candidate.npf[i]
        for i in range(3):
            for j in range(3):
                model[f"r{i}{j}"] = candidate.R[i][j]
        met_obs.calibration_model = model
        met_obs.calibration_residual = candidate.calib_error
        # 测光
        met_obs.photometry_method = None
        met_obs.photometry_model = None
        met_obs.photometry_residual = None
        # ID
        met_obs.station_id = candidate.stationID
        met_obs.meteor_id = None
        return met_obs
